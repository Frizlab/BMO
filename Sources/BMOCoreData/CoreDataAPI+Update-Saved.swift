...
public extension CoreDataAPI {
	...
	func update<Object : NSManagedObject>(
		...
		autoStart: Bool = true,
		savedUpdates: @escaping @Sendable (_ object: Object, _ managedObjectContext: NSManagedObjectContext) throws -> Void,
		...
		return try localDb.context.performAndWaitRW{
			guard let object = try localDb.context.existingObject(with: objectID) as? Object else {
				throw Err.invalidObjectType
			}
			try savedUpdates(object, localDb.context)
			
			let bridgeRequest = settings.updateObjectBridgeRequest(object, .saveBeforeGoingRemote)
