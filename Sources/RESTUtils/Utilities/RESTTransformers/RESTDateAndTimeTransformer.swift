...
/* Note: This transformer is only available when `ISO8601DateFormatter` is available.
 * In theory, only the `iso8601` case of the `DateConversionFormat` enum should be marked as unavailable when the formatter is unavailable.
...
public final class RESTDateAndTimeTransformer : ValueTransformer {
	...
	public enum DateConversionFormat {
		...
		/**
		 Useful for parsing birthdates for instance; expect a date with format `yyyy-MM-dd`.
		 If match, will return the given date at 0:00 GMT. */
		case dateNoTime(locale: Locale?)
		
		/**
		 If you want to mix `dateNoTime` and `iso8601`, put `dateNoTime` first as `iso8601` will parse `dateNoTime` successfully, but with a (probably) wrong timezone. */
	...
	/**
	 Try and convert the given object with the given formats.
	 Try the conversions in the order given for the formats. */
