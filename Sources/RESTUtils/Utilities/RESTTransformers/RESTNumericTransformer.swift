...
/**
 Uses a Scanner to convert an object into a Number (float, double or int depending on the configuration of the transformer).
 
 Does not allow reverse transformation because there are no real sane default value for converting a numeric value to a String
  (except for Ints, of course, but this is a generic transformer for all numeric values). */
