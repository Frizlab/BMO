 ...
 *
 * When saving a Db context through a BMORequestManager, the manager will query its backing store to save the changes detected in the Db context.
 * Similarly, when executing a Db fetch request through the BMO manager, the request will be executed synchronously first,
 *  then the manager will contact its backing store to retrieve new object if applicable.
 * The retrieved objects will be inserted in the Db context and the client will be called asynchronously to be notified of the update.
 *
 * This principle historically comes from Apple documentation about the incremental data store.
 * In theory, with incremental data stores, one should be able to hook Core Data to a WebService (so they say).
 * In practice, this would be done using a separate Core Data stack to store the results of previous requests from the WebService.
 * When a fetch request is done on such a stack, the incremental store would first query its private stack, and return the results directly.
 * Just before that, it would have launched a web operation.
 * When the operation finishes, it will send a notification so that concerned clients can re-fetch their data
 *  (if possible blocking the creation of a web operation to avoid infinite requests to the backing store…).
 * Even though it's not very practical, this solution _could_, at a first glance, work.
 * However, in practice, this solution is **not** compatible with NSFetchedResultsController.
 * (When the web operation finishes, the private CD stack is updated, but the client context has no notification to be informed of the changes.
 *  Re-fetching the new objects won't trigger the required notifications for the fetched results controller to be aware new objects are available in the context!
 *  Even refreshing the objects (which is in itself an expensive operation) does not work if I recall correctly…)
 * After some thinking, I came up with the FLCAISRequestManager, which is the ancestor of the BMORequestManager.
 * FYI, here is an implementation of the original “async incremental store” (the project has been archived):
...
public final class RequestManager {
	...
	public static var defaultBackOperationQueue: OperationQueue {
		 ...
		 * We want this queue to be used on operations that use URL Session and store their result in memory (said later in doc).
		 * We don't really need to limit the number of concurrent requests as the session correctly manages its resources, and writing stuff to memory is a lightweight operation.
		 *
		 * Limiting at Int.max works.
		 * However, testing showed limiting to a lower value results in a smoother application (main threads blocks less) and actually faster processing in general. */
	...
	public static var defaultParseOperationQueue: OperationQueue {
		...
		/* We use the default max concurrent operation count which should be equal to the best possible count for CPU operations depending on the current hardware.
...
//		if let n = numberOfCores {ret.maxConcurrentOperationCount = n}
	...
	/**
	 Helper to find a bridge if none is given when creating the operation for a given request with the request manager (automatic bridge retrieving).
	 
	 Required **only** if you plan on using the automatic bridge retrieving, which is not mandatory. */
	...
	/**
	 ...
	 - Parameter backOperationQueue: The queue on which the back operations (API calls) will be made.
	 If nil, a default queue (well suited for back operations using URL Session) will be used. */
	...
	/**
	 Creates and returns the operation for the given Db request.
	 The operation can be executed only once.
	 If you did not start it automatically when creating the operation, you can simply start it with `op.start()`, or by adding it to a queue.
	 If you want to start the operation manually with `start()`, you’ll have to make sure all dependencies are finished before starting the operation.
	 (As you would for any other operations.)
	 The returned operation is guaranteed to have no dependencies.
	 You can add dependencies if `autoStart` was set to `false`.
	 
	 You can also cancel the operation at any time with `op.cancel()`.
	 
	 - Important: The completion handler given in this method is called from the completion block of the operation.
	 Do **not** change the completion block of the operation if you expect the handler to be called.
	 
	 - Parameter bridge: The bridge to use to create and parse the API request/results.
	...
	public func operation<Request, Bridge>(forBackRequest request: Request, withBridge bridge: Bridge? = nil, resultsImporterFactory: AnyBackResultsImporterFactory? = nil, autoStart: Bool, handler: ((_ response: Result<BackRequestResult<Request, Bridge>, Error>) -> Void)? = nil) -> BackRequestOperation<Request, Bridge> {
		...
		if let handler = handler {
			operation.completionBlock = {
				operation.completionBlock = nil /* To test: Can this be removed? */
